                      ...
                      <tbody>
                        {rows.map((row) => (
                          <tr className="border-b border-slate-200 hover:bg-slate-50/60" key={row.id}>
                            ...
                            <td className="border-r border-slate-200 p-1.5">
                              <input className={timeInputClass} type="time" value={row.planned_start} onChange={(e) => updateSodRow(row.id, 'planned_start', e.target.value)} />
                            </td>
                            <td className="border-r border-slate-200 p-1.5">
                              <input className={timeInputClass} type="time" value={row.planned_end} onChange={(e) => updateSodRow(row.id, 'planned_end', e.target.value)} />
                            </td>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Update actual start/end times for each task and submit.</p>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 bg-slate-100 rounded-lg px-3 py-1.5">
                    <Clock size={13} />
                          ...
                          <tbody>
                            {eodRows.map((row) => (
                              <tr key={row.id} className="border-b border-slate-200 hover:bg-orange-50/20">
                                ...
                                <td className="border-r border-slate-200 p-1.5">
                                  <input className={timeInputClass} type="time" value={row.actual_start} onChange={(e) => updateEodRow(row.id, 'actual_start', e.target.value)} />
                                </td>
                                <td className="border-r border-slate-200 p-1.5">
                                  <input className={timeInputClass} type="time" value={row.actual_end} onChange={(e) => updateEodRow(row.id, 'actual_end', e.target.value)} />
                                </td>
