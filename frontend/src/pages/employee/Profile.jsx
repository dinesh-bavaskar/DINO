      ...
      <main className="flex-1 overflow-auto bg-slate-50 p-4 md:p-7">
        {loading ? <Loader text="Loading profile..." /> : (
          <div className="mx-auto max-w-3xl space-y-5">
            <section className="relative overflow-hidden rounded-lg bg-blue-900 p-7 text-white shadow-sm">
              <div className="absolute -right-8 -top-8 h-40 w-40 rounded-full bg-white/10" />
              <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center">
                <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-2 border-white/30 bg-white/20 text-3xl font-semibold overflow-hidden">
                  {profile?.profile_photo && typeof profile.profile_photo === 'string' ? (
                    <img src={profile.profile_photo.startsWith('http') ? profile.profile_photo : `http://127.0.0.1:8000${profile.profile_photo}`} alt="Profile" className="h-full w-full object-cover" />
                  ) : (
                    (profile?.full_name || user?.full_name || 'U').charAt(0).toUpperCase()
                  )}
                </div>
                <div>
                  <h2 className="text-2xl font-semibold">{profile?.full_name || user?.full_name}</h2>
            ...
            <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm flex flex-col">
              <div className="border-b border-slate-200 px-6 py-4 relative z-10 shadow-sm bg-white">
                <h3 className="font-medium text-slate-950 text-base">Personal Details</h3>
              </div>
