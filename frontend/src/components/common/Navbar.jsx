...
const Navbar = ({ title, subtitle }) => {
  ...
  return (
    <header className="sticky top-0 z-10 shrink-0 border-b border-border bg-background shadow-sm">
      <div className="flex min-h-16 items-center justify-between gap-4 px-4 md:px-7">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col items-center justify-center bg-slate-50/70 border border-slate-200/60 rounded-xl px-5 py-1.5 shadow-none select-none">
            <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider leading-none mb-1">
              CURRENT TIME
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-semibold text-primary-foreground overflow-hidden">
              {user?.profile_photo && typeof user.profile_photo === 'string' ? (
                <img src={user.profile_photo.startsWith('http') ? user.profile_photo : `http://127.0.0.1:8000${user.profile_photo}`} alt="Profile" className="h-full w-full object-cover" />
              ) : (
